...
object IotAnalysisBak {
  def main(args: Array[String]): Unit = {
    ...
    source.filter(line =>line.substring(0, 2).equals("T1"))
      ...
      .aggregate(new AvgTempFunction, new AvgProcessFunction)
      .print()
...
// 编写UDF， 实现对全天温度的统计。将增量聚合函数的输出，作为全量聚合函数的输入
class AvgProcessFunction extends ProcessWindowFunction[(String, Double, Int), String, String, TimeWindow] {
  override def process(key: String, context: Context, elements: Iterable[(String, Double, Int)], out: Collector[String]): Unit = {
